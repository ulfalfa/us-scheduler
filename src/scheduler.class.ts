...
import { map, delay, expand, skip } from 'rxjs/operators'
...
export interface ComplexTime {
  time: string // time can be either a labeled time (eg. dusk or midnight) or a string in format hh24:mm
  random?: number // a randomizing factor in minutes
  offset?: number // a potential offset in minutes
}
export type SimpleTime = string
export type TimeDefinition = SimpleTime | ComplexTime
...
function resolveSimpleTime(event: SimpleTime, curTimes: Times): DateTime {
  if (curTimes[event]) {
    ...
    if (match) {
      ...
      return curTimes[START_LABEL].set({
...
export class UsScheduler {
  ...
  resolve(time: TimeDefinition): any {
    ...
    if (curTimes[event]) {
      ...
      if (match) {
        ...
        throw new Error(
          `${event} is neither predefined nor a valid time (HH:mm)`
        )
      }
    }
  }
  ...
  *generateDaySchedule(
    events: SimpleTime[] | SchedulingOptions
  ): IterableIterator<ScheduleEvent> {
    const options: SchedulingOptions = Array.isArray(events)
      ? { events, random: 0, dayCronPattern: '* * *' }
      : events
    ...
    const lastTarget = times[START_LABEL].minus({ seconds: 1 })

    while (true) {
      // generate the day (sorted by start )
      const daySchedule = options.events
        .map((event, index) => {
          let result = resolveSimpleTime(event, times)
          if (options.random > 0) {
            const minutes =
              Math.floor(Math.random() * (2 * options.random + 1)) -
              options.random
            result = result.plus({ minutes })
          }
          return {
            date: result,
            index,
  ...
  schedule(
    ...
    return of({
      ...
      expand((current, idx) => {
        ...
        return of(next).pipe(
          delay(next.date.valueOf() - current.date.valueOf())
        )
      }),
      skip(1)
    )
